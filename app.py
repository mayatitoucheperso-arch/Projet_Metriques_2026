...
@app.get("/versailles")
def api_versailles():
    url = "https://api.open-meteo.com/v1/forecast?latitude=48.8049&longitude=2.1204&hourly=relative_humidity_2m"
    response = requests.get(url)
    data = response.json()
    ...
    n = min(len(times), len(humidites))
    result = [
        {"datetime": times[i], "humidite": humidites[i]}
        for i in range(n)
    ]
